...
load("R/straying.RData")
  ...
  empty()
    ...
    for (y in 1:10) {
        ...
        for (Pop.e in 1:4){
          # 1: population of origin
          # Pop.e: emigrate to population Pop.e
          if(Pop.e == 1) { 
            next 
            } else {
          emfile <- paste("tmp/Mig_",1,"-",Pop.e,"_",y,".txt",sep="")
          #pstray <- c(0.1,0.1)
          pstray <- c(mstray[1, Pop.e], mstray[1, Pop.e])
        ...
        for (Pop.i in 1:4){
          ...
          imfile <- paste("tmp/Mig_",Pop.i,"-",1,"_",y,".txt",sep="")
    ...
    if (plotting) {
        pdf(paste('tmp/Res_Pop',1,'.pdf',sep=''))
        op <- par(mfrow = c(2, 2))
