...
load("R/straying.RData")
  ...
  empty()
    ...
    for (y in 1:10) {
        ...
        for (Pop.e in 1:4){
          # 3: population of origin
          # Pop.e: emigrate to population Pop.e
          if(Pop.e == 3) { 
            next 
            } else {
          emfile <- paste("tmp/Mig_",3,"-",Pop.e,"_",y,".txt",sep="")
          #pstray <- c(0.1,0.1)
          pstray <- c(mstray[3, Pop.e], mstray[3, Pop.e])
        ...
        for (Pop.i in 1:4){
          ...
          imfile <- paste("tmp/Mig_",Pop.i,"-",3,"_",y,".txt",sep="")
    ...
    if (plotting) {
        pdf(paste('tmp/Res_Pop',3,'.pdf',sep=''))
        op <- par(mfrow = c(2, 2))
